...
namespace :env do
  ...
  task :set_dirs do
    ...
    FOLDERS[:rb][:out] = File.join(FOLDERS[:src], PROJECTS[:rb][:dir], 'bin', CONFIGURATION)
    CLEAN.include(FOLDERS[:rb][:out])

    # for tests
    FOLDERS[:rb][:test_out] = File.join(FOLDERS[:src], PROJECTS[:rb][:test_dir], 'bin', CONFIGURATION)
    FILES[:rb][:test] = File.join(FOLDERS[:rb][:test_out], "#{PROJECTS[:rb][:test_dir]}.dll")
